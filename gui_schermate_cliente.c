...
/*
 * gui_schermate_cliente.c
 * Schermate area cliente.
 */
/* HOME */
...
void schermata_feedback(AppState *s)
{
    ...
    if (bottone_accent(cx - SC(130), y, SC(260), BTN_H, "SALVA FEEDBACK")) {
        ...
        /* Ricalcola programma e piano con i dati aggiornati (peso, obiettivo,
           livello attivita', kcal) cosi' che Schede/Piani mostrino subito
           il contenuto corretto senza dover rifare login. */
        s->programma = costruisci_programma(u);
        s->piano     = costruisci_piano(u);
        s->programma_pronto = 1;
        s->piano_pronto     = 1;

        /* Forza il ricaricamento dei dati: la prossima volta che il cliente apre
           Schede o Piani, il buffer viene ricostruito dal nuovo programma/piano
           invece di mostrare quello calcolato al login. */
