...
int main(void)
{
    ...
    while (!WindowShouldClose()) {
        ...
        /* Storico pesi cliente
             Caricato come lista collegata dinamica (malloc/free),
             poi formattato in testo per la GUI e la lista liberata. */
        ...
        /* Schede cliente
             Mostra la variante assegnata dal trainer (se presente),
             altrimenti il programma base generato al login. */
        case SCHERMATA_SCHEDE: {
            if (stato.ultima_caricata != SCHERMATA_SCHEDE) {
                ...
                if (stato.utente.servizio != SERVIZIO_SOLO_NUTRIZIONE) {
                    /* Controlla se il trainer ha assegnato una variante */
                    ...
                    if (var >= 0) {
                        ...
                        /* Nessuna assegnazione -> programma base da login */
        ...
        /* Piani cliente
             Mostra la variante assegnata dal trainer (se presente),
             altrimenti il piano base generato al login. */
        ...
        /* Lista clienti: array dinamico (malloc/realloc/free)
             Cresce automaticamente man mano che si leggono i clienti
             dal database, poi viene formattato in testo e liberato. */
