...
/*
 * gui_schermate_trainer.c
 * Schermate area trainer.
 */
...
/* HELPER: campo ricerca cliente + bottone Cerca
 *  Lavora su buffer passati per puntatore (provenienti da AppState,
 *  cosi' lo stato sopravvive solo finche' la sessione e' attiva e
 *  viene azzerato automaticamente al logout). */
...
/* AREA TRAINER - Gestione (solo generazione automatica)
 *  Stato di ricerca/varianti vive in AppState -> azzerato al logout. */
